...
bool RH_RF69::printRegister(Serial &pc, uint8_t reg)
{
#ifdef RH_HAVE_SERIAL
    Serial.print(reg, HEX);
    Serial.print(" ");
    Serial.println(spiRead(reg), HEX);
#elif defined (RH_HAVE_PC)
	pc.printf("%#04x %#04x\n", reg, spiRead(reg));
#endif
    return true;
}

bool RH_RF69::printRegisters(Serial &pc)
{
    uint8_t i;
    for (i = 0; i < 0x50; i++)
	printRegister(pc, i);
    // Non-contiguous registers
	printRegister(pc, RH_RF69_REG_58_TESTLNA);
	printRegister(pc, RH_RF69_REG_6F_TESTDAGC);
	printRegister(pc, RH_RF69_REG_71_TESTAFC);
