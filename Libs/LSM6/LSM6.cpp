...
{
	char cmd[7] = { OUTX_L_XL, 0, 0, 0, 0, 0, 0 };
	//Wire.beginTransmission(address);
...
//	uint8_t zha = Wire.read();
	...
	i2c.read(address, &cmd[1], 6);
	
	  // combine high and low bytes
	acc.x = (int16_t)(cmd[2] << 8 | cmd[1]);
	acc.y = (int16_t)(cmd[4] << 8 | cmd[3]);
	acc.z = (int16_t)(cmd[6] << 8 | cmd[5]);
}

// Reads the 3 gyro channels and stores them in vector g
void LSM6::readGyro(void)
{
	char cmd[7] = { OUTX_L_G, 0, 0, 0, 0, 0, 0 };
//	Wire.beginTransmission(address);
//	// automatic increment of register address is enabled by default (IF_INC in CTRL3_C)
//	Wire.write(OUTX_L_G);
//	Wire.endTransmission();
...
//	uint8_t zhg = Wire.read();
	...
	i2c.read(address, &cmd[1], 6);
	  ...
	  // combine high and low bytes
	gyro.x = (int16_t)(cmd[2] << 8 | cmd[1]);
	gyro.y = (int16_t)(cmd[4] << 8 | cmd[3]);
	gyro.z = (int16_t)(cmd[6] << 8 | cmd[5]);
}
